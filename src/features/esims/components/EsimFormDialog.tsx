...
function FormInput({ label, name, type = 'text', value, required, saving, error, min, step, maxLength, onChange }: FormInputProps) { const errorId = error ? `${name}-error` : undefined; return <label className="grid gap-2"><span className="text-sm font-medium text-gray-700 dark:text-gray-200">{label}</span><Input name={name} type={type} value={value} required={required} disabled={saving} min={min} step={step} maxLength={maxLength} hasError={Boolean(error)} aria-invalid={Boolean(error)} aria-describedby={errorId} onChange={(event) => onChange(event.target.value)} />{error && <span id={errorId} className="text-sm text-red-600 dark:text-red-400">{error}</span>}</label> }
...
function FormSelect({ label, name, value, required, saving, error, onChange, children }: FormSelectProps) { const errorId = error ? `${name}-error` : undefined; return <label className="grid gap-2"><span className="text-sm font-medium text-gray-700 dark:text-gray-200">{label}</span><SelectNative name={name} value={value} required={required} disabled={saving} hasError={Boolean(error)} aria-invalid={Boolean(error)} aria-describedby={errorId} onChange={(event) => onChange(event.target.value)}>{children}</SelectNative>{error && <span id={errorId} className="text-sm text-red-600 dark:text-red-400">{error}</span>}</label> }
