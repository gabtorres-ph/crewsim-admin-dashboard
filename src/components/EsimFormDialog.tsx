...
export function EsimFormDialog(props: EsimFormDialogProps) {
  ...
  </div>{error && <div role="alert" className="mb-5 rounded-md border border-red-200 bg-red-50 p-3 text-sm text-red-700">{error}</div>}<DialogFooter><DialogClose asChild><Button type="button" variant="secondary" disabled={saving}>Cancel</Button></DialogClose><Button type="submit" isLoading={saving} loadingText="Saving">{mode === 'add' ? 'Add eSIM' : 'Save changes'}</Button></DialogFooter></form></DialogContent></Dialog>
}
